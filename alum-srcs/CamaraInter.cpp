...
void CamaraInteractiva::calcularViewfrustum(  )
{
  ...
  // Calcular los parámetros del view frustum (vf), y actualiza la matriz de proyección (vf.matrizProy)
  if ( perspectiva ) {
...
void CamaraInteractiva::moverHV( float nh, float nv )
{
  ...
  {
    // desplazar 'mcv.org' en X e Y y recalcular la matriz de vista (y movimiento solidario del punto de atención)
    mcv.org(X) += nh * udesp;
    mcv.org(Y) += nv * udesp;
...
void CamaraInteractiva::desplaZ( float nz )
{
  ...
  {
    // desplazar 'mcv.org' en Z, y recalcular la matriz de vista (y movimiento solidario del punto de atención)
    mcv.org(Z) -= nz * udesp;
    aten(Z) -= nz * udesp;
    recalcularMatrMCV();
  }
