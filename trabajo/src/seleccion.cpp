...
bool Seleccion( int x, int y, Escena * escena, ContextoVis & cv_dib )
{
   ...
   Tupla3f centro_wc;
   bool encontrado = escena->objetoActual()->buscarObjeto(id_pixel, mmodelado, objeto, centro_wc);
   if(!encontrado){
     cout << "No se encuentra el objeto." << endl;
     return false;
   }
   cout << "Seleccionado el objeto: " << (*objeto)->leerNombre() << endl;
   cout << "Identificador del objeto: " << (*objeto)->leerIdentificador() << endl;
