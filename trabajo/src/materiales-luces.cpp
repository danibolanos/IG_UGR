...
void ColFuentesLuz::activar( Cauce & cauce )
{
  ...
  std::vector<Tupla4f> posdir;
  Tupla4f tmp;
  float lo, lt;
  for(int i=0; i<vpf.size(); i++){
    colors.push_back(vpf[i]->col);
    lo = vpf[i]->longi;
    lt = vpf[i]->lati;
    tmp = {cos(float(lo*M_PI/180.0)), sin(float(lt*M_PI/180.0)), sin(float(lo*M_PI/180.0)), 0.0};
    posdir.push_back(tmp.normalized());
  }
  cauce.fijarFuentesLuz(colors, posdir);
}
