...
void Camara3Modos::desplRotarXY( const float da, const float db )
{
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         double valor = 0.5*M_PI-0.01;
         org_polares[0] -= 0.02*da;
       	 org_polares[1] += 0.02*db;

       	 if( org_polares[1]< -valor )
           org_polares[1] = -valor;
       	 else if( org_polares[1] > valor )
           org_polares[1] = valor;
      ...
      case ModoCam::prim_pers_rot :
      {
         ...
         org_polares[0] += 0.02*da;
       	...
       	Tupla3f ant_cartesianas = org_cartesianas;
       	org_cartesianas = Cartesianas(org_polares);
       	Tupla3f dsp = ant_cartesianas - org_cartesianas;
        punto_atencion = punto_atencion + dsp;
        break ;
      }
      case ModoCam::prim_pers_despl :
      {
         ...
         Tupla3f dsp = -da*eje[X]*0.5+db*eje[Y]*0.5;
         punto_atencion = punto_atencion + dsp;
         org_cartesianas = org_cartesianas + dsp;
         org_polares = Esfericas(org_cartesianas);
         break ;
      }
   }
...
void Camara3Modos::moverZ( const float dz )
{
   ...
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         float min = 0.5;
       	 org_polares[Z] = min+(org_polares[Z]-min)*pow(1+0.1,dz);
       	 org_cartesianas = Cartesianas(org_polares);
         break ;
      }
      case ModoCam::prim_pers_rot :
